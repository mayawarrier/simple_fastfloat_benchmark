...
fastfloat_really_inline FASTFLOAT_CONSTEXPR20
void parse_if_eight_digits_loop(const UC*& p, const UC* const pend, uint64_t& i) {
  if (!has_simd_opt<UC>()) {
    return;
  }
...
fastfloat_really_inline FASTFLOAT_CONSTEXPR20
void parse_if_eight_digits_loop(const char*& p, const char* const pend, uint64_t& i) {
  // optimizes better than parse_if_eight_digits_unrolled() equivalent for UC = char.
...
parsed_number_string_t<UC> parse_number_string(UC const *p, UC const * pend, parse_options_t<UC> options) noexcept {
  ...
  if ((p != pend) && (*p == decimal_point)) {
    ...
    //  p += 8;
    //}
    parse_if_eight_digits_loop(p, pend, i);
